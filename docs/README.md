# Bleak Bumble Documentation

This directory contains the documentation for `bleak_bumble`, an out-of-tree Bumble backend for Bleak.

## Documentation Files

- [Installation](installation.md) - Installation instructions and requirements
- [Usage](usage.md) - Basic usage and configuration
- [Examples](examples.md) - Practical examples with different transport layers
- [API Reference](api.md) - API documentation and reference

## Quick Links

- [GitHub Repository](https://github.com/vChavezB/bleak-bumble)
- [Bumble Project](https://github.com/google/bumble)
- [Bleak Project](https://github.com/hbldh/bleak)
